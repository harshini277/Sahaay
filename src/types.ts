...
export type Grievance = { id:string; citizenId:string; title:string; category:string; issue:string; authority:string; status:Status; expected:string; submittedAt?:string; events:Event[]; document?:boolean; feedback?:string; rating?:number; appealId?:string; appealReason?:string; submission?:SubmissionDetails }
export type Citizen = { id:string; name:string; email:string; phone:string; state:string; district:string; address:string; greeting:string }
